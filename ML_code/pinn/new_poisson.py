...
class PINN(torch.nn.Module):
    ...
    def loss(self, x, u):
        ...
        # 物理约束
        u_pred = self.forward(torch.FloatTensor(x.size()).fill_(1)) # 神经网络预测的u值
        u_pred_x = torch.autograd.grad(u_pred, x, create_graph=True)[0] # 对x求导得到u_x值
        u_pred_xx = torch.autograd.grad(u_pred_x, x, create_graph=True)[0] # 对x求导得到u_xx值
